...
use wasmedge_sdk::{host_function, Vm, config::{CommonConfigOptions, ConfigBuilder, HostRegistrationConfigOptions}, Caller, WasmValue, error::HostFuncError, ValType, ImportObjectBuilder, params, WasmVal};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let res = Vm::new(Some(config))?.register_import_module(import)?.register_module_from_file("extern", wasm_lib_file)?.run_func(Some("extern"), "add", params!(num1, num2))?;
